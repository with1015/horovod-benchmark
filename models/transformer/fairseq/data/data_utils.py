...
class EpochBatchIterator(object):
    ...
    def __init__(
        ...
        with numpy_seed(self.seed):
            ...
            if not (self.max_tokens is  float('Inf')): # Set up batch size w.r.t sentences
                if required_batch_size_multiple == 1: # Train
                    # [GC] This is the same as train-max-length in GNMT
                    # We force max length to 50 to prevent increasing memory usage
                    max_positions_num = 50
                else:
                    # [GC] This is the same as test-max-length in GNMT
                    # Score BLEU to resolve the error:
                    # Source and reference streams have different lengths
                    max_positions_num = 150
            else:
                max_positions_num = 1024 # original code
    ...
    def _get_iterator_for_epoch(self, epoch, shuffle):
        ...
        # [GC] Set num_workers to 0
        # to resolve TypeError: cannot serialize '_io.BufferedReader' object
        # when using torch.multiprocessing
