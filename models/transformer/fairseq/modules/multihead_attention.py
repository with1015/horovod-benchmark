...
class QueryLinear(torch.autograd.Function) :
    ...
    def forward(ctx, input, weights_q, scale) :
        ...
        q = torch.addmm(input.contiguous().view(input.size(0)*input.size(1), input.size(2)), input.contiguous().view(input.size(0) * input.size(1), input.size(2)), weights_q, beta=0.0, alpha=s[0])
...
class KeyValueLinears(torch.autograd.Function) :
    ...
    def forward(ctx, input, weights_k, weights_v) :
        ...
        k = torch.addmm(input.contiguous().view(input.size(0)*input.size(1), input.size(2)), input.contiguous().view(input.size(0) * input.size(1), input.size(2)), weights_k, beta=0.0, alpha=1.0)
        k=k.view(input.size(0), input.size(1), input.size(2))
        v = torch.addmm(input.contiguous().view(input.size(0)*input.size(1), input.size(2)), input.contiguous().view(input.size(0) * input.size(1), input.size(2)), weights_v, beta=0.0, alpha=1.0)
...
def query_linear(input: Tensor, weights_q: Tensor, scale: float):
    if not torch.jit.is_scripting():
        ...
        q = scale * torch.einsum('ij,jk->ik', input.contiguous().view(input.size(0)*input.size(1), -1), weights_q)
...
def key_value_linears(input: Tensor, weights_k: Tensor, weights_v: Tensor):
    if not torch.jit.is_scripting():
        ...
        k = torch.einsum('ij,jk->ik', input.contiguous().view(input.size(0)*input.size(1), -1), weights_k)
        k = k.view(input.shape)
        v = torch.einsum('ij,jk->ik', input.contiguous().view(input.size(0)*input.size(1), -1), weights_v)
