...
def all_gather_list(data, max_size=16384):
    ...
    for i in range(world_size):
        ...
        # https://stackoverflow.com/questions/33307623/python-exception-safe-pickle-use
        try:
            ...
            print(f'[rank:{torch.distributed.get_rank()}] pickle.UnpicklingError: {e}')
        except (AttributeError,  EOFError, ImportError, IndexError) as e:
            # secondary errors
            print(f'[rank:{torch.distributed.get_rank()}] Error: {e}')
        except Exception as e:
            # everything else
            print(f'[rank:{torch.distributed.get_rank()}] Exception: {e}')
    return result
