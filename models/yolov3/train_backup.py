...
parser.add_argument('--algorithm', type=str, default='terngrad',
                            help="Run without optimizer to measure computation only")
...
def main_worker(args):
    ...
    if args.fuse_conv_and_bn:
        ...
        model = Darknet(cfg).to(args.gpu).cuda()
    ...
    for k, v in dict(model.named_parameters()).items():
        ...
        elif "Con2d.weight" in k:
            pg1 += [v]
        else:
            pg0 += [v]

    if args.adam:
        optimer = optim.Adam(pg0, lr = args.hyp["lr0"])
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_path, num_replicas=hvd.size(), rank = hvd.rank())

    # Dataloader
    batch_size = min(batch_size, len(dataset))
    nw = device #min([os.cpu_count(), batch_size if batch_size > 1 else 0, 8])  # number of data loading workers
